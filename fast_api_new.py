...
async def upload_photo(photo: UploadFile = File(...), device_id: str = Form(...), db: Session = Depends(get_db)):
    ...
    is_recognize = 'Нет'
    status = recognize_face_info.get('ststus')
    if status:
        is_recognize = 'Да' if status == 'known' else 'Нет'

    success = recognize_face_info.get('ststus') == 'known'

    return_json = {"success": success, "name": recognize_face_info, "info": recognize_face_info, "user": user}
    ...
    log_confirm = LogConfirm(
        ...
        inout = 'распознование', 
        response = is_recognize,
        json_data = str(return_json)
    )
    db.add(log_confirm)
    db.commit()
...
@app.post("/unconfirm")
async def unconfirm(request: Request, db: Session = Depends(get_db)):
    data = await request.json()
    user_code = data.get("user_code")
    device_id = data.get("device_id")
